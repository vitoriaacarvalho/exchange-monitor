...
- The CLI never connects to your database without explicit consent. Pass `--probe-db` to `yarn prisma orm init` if you want `init` to verify the server version itself.
...
Commit both files to git. When you change your contract, run `yarn prisma contract emit` to update them.
...
```bash
yarn prisma contract emit       # Update contract.json and contract.d.ts
yarn prisma db init             # Create tables in the database
yarn prisma migration status    # Show migration status
```
...
2. Run `yarn prisma contract emit` to regenerate the contract.
